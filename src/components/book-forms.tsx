...
import { createBook, deleteBook, requestBook, updateBook } from "@/app/actions/books";
...
export function EditBookForm({
  ...
  initialValues: EditBookValues;
}) {
  const [state, formAction] = useActionState(updateBook, {
    ...initialBookActionState,
    fieldValues: initialValues,
  });
  const values = { ...initialValues, ...state.fieldValues };
  ...
  return (
    <form className="book-form" action={formAction} noValidate>
      ...
      <div className="book-form__field">
        <label htmlFor="title">Title</label>
        <input id="title" name="title" maxLength={200} defaultValue={values.title} required aria-invalid={Boolean(state.fieldErrors?.title)} />
        {state.fieldErrors?.title ? <span>{state.fieldErrors.title}</span> : null}
      </div>

      <div className="book-form__field">
        <label htmlFor="author">Author</label>
        <input id="author" name="author" maxLength={200} defaultValue={values.author} aria-invalid={Boolean(state.fieldErrors?.author)} />
        {state.fieldErrors?.author ? <span>{state.fieldErrors.author}</span> : null}
      </div>
      ...
      <div className="book-form__row">
        <div className="book-form__field">
          <label htmlFor="categoryId">Category</label>
          <select id="categoryId" name="categoryId" defaultValue={values.categoryId} required aria-invalid={Boolean(state.fieldErrors?.categoryId)}>
            <option value="" disabled>Choose a category</option>
            {categories.map((category) => (
              <option value={category.id} key={category.id}>{category.name}</option>
            ))}
        ...
        <div className="book-form__field">
          <label htmlFor="publishedYear">Published year</label>
          <input id="publishedYear" name="publishedYear" type="number" min={1000} max={2100} defaultValue={values.publishedYear} aria-invalid={Boolean(state.fieldErrors?.publishedYear)} />
          {state.fieldErrors?.publishedYear ? <span>{state.fieldErrors.publishedYear}</span> : null}
        </div>
      </div>
      ...
      <div className="book-form__row">
        <div className="book-form__field">
          <label htmlFor="isbn">ISBN</label>
          <input id="isbn" name="isbn" maxLength={32} defaultValue={values.isbn} aria-invalid={Boolean(state.fieldErrors?.isbn)} />
          {state.fieldErrors?.isbn ? <span>{state.fieldErrors.isbn}</span> : null}
        </div>

        <div className="book-form__field">
          <label htmlFor="condition">Condition</label>
          <select id="condition" name="condition" defaultValue={values.condition} aria-invalid={Boolean(state.fieldErrors?.condition)}>
            <option value="">Not specified</option>
            <option value="new">New</option>
            <option value="like_new">Like new</option>
            <option value="good">Good</option>
      ...
      <div className="book-form__field">
        <label htmlFor="description">Description</label>
        <textarea id="description" name="description" rows={5} maxLength={4000} defaultValue={values.description} aria-invalid={Boolean(state.fieldErrors?.description)} />
        {state.fieldErrors?.description ? <span>{state.fieldErrors.description}</span> : null}
      </div>
...
export function DeleteBookForm({ bookId }: { bookId: number }) {
  const [state, formAction] = useActionState(deleteBook, initialBookActionState);
  ...
  return (
    ...
    >
      <input type="hidden" name="bookId" value={bookId} />
      <button className="workflow-button workflow-button--danger" type="submit">
        Delete Book
      </button>
      {state.message ? (
        <p className="workflow-message workflow-message--error" role="alert">
          {state.message}
        </p>
      ) : null}
    </form>
